...
for line in lines:
    ...
    if int(range1[0]) <= int(range2[0]) and int(range1[1]) >= int(range2[1]): # range1 contains range2
        ...
        rangesAnyOverlap += 1
    elif int(range1[0]) >= int(range2[0]) and int(range1[0]) <= int(range2[1]): #range 1 starts inside range 2 or vice versa
        rangesAnyOverlap += 1
    elif int(range2[0]) >= int(range1[0]) and int(range2[0]) <= int(range1[1]): #range 2 starts inside range 1 or vice versa
        rangesAnyOverlap += 1
